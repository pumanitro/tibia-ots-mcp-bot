...
async def _wait_for_position(bot, expected_pos, timeout, tolerance=0):
...
async def _execute_walk_to(bot, node, prefix=""):
    ...
    bot.log(f"{prefix} walk_to ({target[0]},{target[1]},{target[2]}) dist={dist}")

    for attempt in range(MAX_RETRIES):
        current = bot.position
        dist = _distance(current, target)

        if dist <= WALK_TO_TOLERANCE and current[2] == target[2]:
        ...
        arrived = await _wait_for_position(bot, target, timeout, tolerance=WALK_TO_TOLERANCE)
    ...
    return dist <= WALK_TO_TOLERANCE + 2
...
async def _execute_use_item_node(bot, node, prefix=""):
    ...
    if is_floor_change:
        # Wait for z to change
        timeout = USE_ITEM_TIMEOUT
        arrived = await _wait_for_position(bot, target, timeout, tolerance=0)
        if not arrived:
            # For floor changes, just check that z changed
            after = bot.position
            if after[2] != current[2]:
                bot.log(f"{prefix}   floor changed -> ({after[0]},{after[1]},{after[2]})")
                return True
            bot.log(f"{prefix}   missed floor change: still at z={after[2]}")
            return False
        after = bot.position
        bot.log(f"{prefix}   -> ({after[0]},{after[1]},{after[2]})")
        return True
    else:
        # Same-floor interaction (door etc) — brief wait for position change
        await bot.sleep(0.5)
        after = bot.position
        bot.log(f"{prefix}   -> ({after[0]},{after[1]},{after[2]})")
        return True
...
async def _execute_use_item_ex_node(bot, node, prefix=""):
    ...
    after = bot.position
    if not arrived:
        bot.log(f"{prefix}   missed: at ({after[0]},{after[1]},{after[2]})")
    else:
        bot.log(f"{prefix}   -> ({after[0]},{after[1]},{after[2]})")
    return arrived
...
async def run(bot):
    ...
    while state.playback_active:
        ...
        for i, node in enumerate(actions_map):
            ...
            if ntype == "walk_to":
                success = await _execute_walk_to(bot, node, prefix)
            elif ntype == "use_item":
