"""Mana Recovery 70%: uses Senzu (item 8465) when mana drops below 70%."""
import sys
import os
import time
import traceback
        ...
        try:
            if bot.is_connected:
                ...
                if (now - last_state_log) >= STATE_LOG_INTERVAL:
                    bot.log(f"Mana Recovery 70%: state MP={mp} connected={bot.is_connected}")
                    last_state_log = now
                ...
                if mp <= MANA_THRESHOLD and (now - last_use) >= COOLDOWN:
                    ...
                    last_use = now
                    bot.log(f"Mana Recovery 70%: used Senzu (MP={mp})")
        except Exception as e:
            bot.log(f"Mana Recovery 70% ERROR: {e}")
            traceback.print_exc()
